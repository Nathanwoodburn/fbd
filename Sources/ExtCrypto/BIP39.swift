...
public enum BIP39 {
    ...
    public static func generateMnemonic(strength: Int = 256) -> String {
        ...
        #if canImport(Security)
        // macOS/iOS: use SecRandomCopyBytes (CSPRNG)
        ...
        defer { fclose(fp) }
        let read = fread(&entropy, 1, entropy.count, fp)
        precondition(read == entropy.count, "/dev/urandom short read")
        #elseif canImport(WinSDK)
        // Windows: BCryptGenRandom (CSPRNG)
        guard BCryptGenRandom(nil, &entropy, ULONG(entropy.count), ULONG(BCRYPT_USE_SYSTEM_PREFERRED_RNG)) == 0 else {
            preconditionFailure("BCryptGenRandom failed")
        }
        #else
        // Fallback: platform random
        for i in 0..<entropy.count { entropy[i] = UInt8.random(in: 0...255) }
        #endif
        return mnemonicFromEntropy(entropy)
    }

    /// Convert entropy bytes to a mnemonic phrase.
    public static func mnemonicFromEntropy(_ entropy: [UInt8]) -> String {
        let checksumBits = entropy.count / 4 // 1 bit per 32 bits of entropy
        let checksum = Array(Crypto.SHA256.hash(data: entropy))
        var bits = [Bool]()
        ...
        for j in 0..<checksumBits {
            bits.append((checksum[0] >> (7 - j)) & 1 == 1)
        }
        ...
        for i in 0..<wordCount {
            ...
            words.append(englishWordlist[index])
        }
    ...
    /// - Returns: 64-byte seed for BIP32 key derivation.
    public static func toSeed(mnemonic: String, passphrase: String = "") -> [UInt8] {
        ...
        return PBKDF2.sha512(
            ...
            salt: Array(normalizedSalt.utf8),
            rounds: 2048,
            keyLength: 64
        )
    }

    /// Validate a mnemonic phrase (word count, wordlist membership, checksum).
    public static func validateMnemonic(_ mnemonic: String) -> Bool {
        ...
        // Check all words exist and collect indices
        var indices = [Int]()
        for word in words {
            guard let idx = wordlistIndex[word] else { return false }
            indices.append(idx)
        }
        ...
        for idx in indices {
            for j in (0..<11).reversed() {
                bits.append((idx >> j) & 1 == 1)
            }
        }

        let checksumBits = words.count / 3 // CS = ENT/32, ENT = words*11 - CS → CS = words/3
        ...
        for i in 0..<checksumBits {
            let expected = (checksum[0] >> (7 - i)) & 1 == 1
    ...
    private static let wordlistIndex: [String: Int] = {
        var map = [String: Int]()
        map.reserveCapacity(2048)
        for (i, word) in englishWordlist.enumerated() {
            map[word] = i
        }
        return map
    }()
